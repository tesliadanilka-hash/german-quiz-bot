...
async def cmd_start(message: Message) -> None:
    ...
    # Сбрасываем прогресс по словам, чтобы новый круг начинался заново
    reset_progress(uid)
    ...
    text = (
        ...
        "• Выбираешь правило уровня A1.\n"
        "• Сначала идет объяснение с примерами немецкий + перевод на русский.\n"
        "• Потом мини тест из 5 вопросов с 4 вариантами ответа.\n"
        "• После окончания теста показывается статистика по этому правилу.\n\n"
        f"Правил уровня A1 сейчас: {total_rules}.\n\n"
        "Команды:\n"
        "/next  начать тренировать слова\n"
        "/themes  выбрать тему слов\n"
        "/mode  выбрать направление перевода\n"
        "/grammar  выбрать грамматическое правило A1 и пройти тест\n\n"
        "Сначала прочитай эту информацию, а потом выбери команду, чтобы начать."
    )

    await message.answer(text)
    # ВАЖНО: не вызываем здесь send_new_word
