...
# В файле темы могут быть вида "A1: Приветствия и базовые фразы"
...
user_state: Dict[int, Dict[str, Any]] = defaultdict(
    lambda: {
        ...
        "awaiting_correction": False,  # ждем ли сейчас текст для проверки
    }
)
...
def normalize_topic_name(raw: str) -> str:
    """
    Убираем уровни вида 'A1:' или 'A2:' и пробелы.
    Пример: 'A1: Приветствия и базовые фразы' -> 'Приветствия и базовые фразы'.
    """
    raw = raw.strip()
    if ":" in raw:
        # берем все после первого двоеточия
...
def load_words(path: str = "words.json") -> None:
    ...
    Темы берутся из файла и маппятся через TOPIC_NAME_MAP.
    """
    ...
    with file_path.open("r", encoding="utf-8") as f:
        ...
        def add_word(raw: Dict[str, Any], topic_raw: str) -> None:
            """
            Добавляет одно слово. Тема берется из файла и переводится
            в внутренний формат через TOPIC_NAME_MAP.
            """
            de = raw.get("de")
            tr = raw.get("tr")
            ru = raw.get("ru")
            ...
            normalized = normalize_topic_name(topic_raw)
            topic = TOPIC_NAME_MAP.get(normalized, TOPIC_DICT)
...
# ==========================
# ПРОВЕРКА НЕМЕЦКОГО ТЕКСТА
# ==========================
# Эта часть отвечает за новую функцию:
# ты пишешь предложение на немецком, бот отправляет его в LanguageTool
# и возвращает исправленный текст плюс список ошибок.

def build_corrected_text(original: str, matches: List[Dict[str, Any]]) -> str:
    """
    Собираем исправленный текст, подставляя первое предлагаемое исправление
    для каждого найденного LanguageTool совпадения.
    """
    corrected = original
    # идем с конца, чтобы не сбивать offset
    for m in sorted(matches, key=lambda x: x.get("offset", 0), reverse=True):
        ...
        if not repls:
            continue
        new_val = repls[0].get("value", "")
        corrected = corrected[:offset] + new_val + corrected[offset + length:]
    return corrected


async def check_german_text(text: str) -> str:
    """
    Отправляем текст в LanguageTool и возвращаем красивый ответ
    с исправленным вариантом и списком ошибок.
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=data) as resp:
            try:
                ...
                return "Не получилось проверить текст. Попробуй позже."

    matches = result.get("matches", [])
    if not matches:
        return f"✅ Ошибок не найдено.\n\nТвой текст:\n{text}"

    corrected = build_corrected_text(text, matches)

    lines: List[str] = []
    lines.append("✏ Исправленный вариант:")
    ...
    for i, m in enumerate(matches, start=1):
        ...
        repls = m.get("replacements", [])
        best = repls[0].get("value", "") if repls else ""
        msg = m.get("message", "")
        lines.append(f"{i}. '{wrong}' → '{best}' - {msg}")
...
async def cmd_start(message: Message) -> None:
    ...
    if uid != ADMIN_ID and uid not in allowed_users:
        text = (
            ...
            "• Проверять твои предложения на немецком и показывать ошибки\n\n"
    ...
    text = (
        ...
        "• Проверять твои предложения на немецком и показывать исправления\n\n"
        ...
        "• /grammar - грамматика\n"
        "• /check - проверить твое предложение на немецком и увидеть ошибки\n\n"
...
async def cmd_check(message: Message) -> None:
    """
    Команда /check включает режим проверки текста.
    После нее пользователь отправляет предложение на немецком,
    а бот возвращает исправленный вариант и список ошибок.
    """
    uid = message.from_user.id
    ...
    user_state[uid]["awaiting_correction"] = True
    await message.answer(
        "Отправь мне предложение или небольшой текст на немецком, "
        "я попробую найти и исправить ошибки."
    )
...
async def cb_allow_user(callback: CallbackQuery) -> None:
    ...
    try:
        text = (
            ...
            "• Проверяет твой текст на немецком и показывает исправления\n\n"
            "Режимы тренировки слов:\n"
            ...
            "• /next - следующее слово в текущей теме\n"
            "• /grammar - грамматика\n"
            "• /check - проверка твоего текста на немецком\n\n"
            "Важно:\n"
            "Если ответ по словам неправильный, новое слово не дается.\n"
...
# ==========================
# ОБРАБОТЧИК СВОБОДНОГО ТЕКСТА ДЛЯ /check
# ==========================

@dp.message(F.text & ~F.text.startswith("/"))
async def handle_free_text(message: Message) -> None:
    """
    Этот хендлер реагирует на обычные текстовые сообщения,
    когда пользователь включил режим проверки командой /check.
    Если режим не включен, сообщения игнорируются.
    """
    uid = message.from_user.id
    ...
    if not state.get("awaiting_correction"):
        return
    ...
    if not original:
        await message.answer("Текст пустой. Напиши предложение на немецком.")
        return

    await message.answer("Проверяю текст...")
    result = await check_german_text(original)
