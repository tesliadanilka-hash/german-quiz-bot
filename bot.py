...
# ==========================
# ФУНКЦИИ ДЛЯ ИИ ПРОВЕРКИ ПРЕДЛОЖЕНИЙ
# ==========================
...
async def check_text_with_ai(text: str) -> str:
    if client is None:
        return (
            ...
            "Не настроен API ключ OpenAI. Обратись к администратору."
        )
    ...
    try:
        ...
        return "Произошла ошибка при обращении к ИИ. Попробуй еще раз позже."
...
async def cmd_start(message: Message) -> None:
    ...
    if uid != ADMIN_ID and uid not in allowed_users:
        text = (
            ...
            "• Проверять грамматику твоих предложений на немецком через ИИ\n\n"
    ...
    text = (
        ...
        "• Проверять грамматику твоих предложений на немецком через ИИ (/check)\n\n"
        ...
        "• /grammar - грамматика\n"
        "• /check - включить режим проверки предложений через ИИ\n"
        ...
        "В режиме /check ты пишешь предложения на немецком, а бот показывает исправленный вариант и ошибки.\n\n"
...
async def cmd_check_on(message: Message) -> None:
    ...
    await message.answer(
        ...
        "Я проверю грамматику через ИИ, покажу исправленный вариант и список ошибок.\n\n"
...
async def handle_plain_text(message: Message) -> None:
    ...
    waiting_msg = await message.answer("⌛ Проверяю предложение через ИИ...")
...
async def cb_allow_user(callback: CallbackQuery) -> None:
    ...
    try:
        text = (
            ...
            "• Проверяет грамматику твоих предложений на немецком через ИИ\n\n"
            "Режимы тренировки слов:\n"
            ...
            "В режиме /check ты просто пишешь предложения на немецком, а бот показывает исправленный вариант и ошибки."
        )
