...
import aiohttp  # ДОБАВЛЕНО: для HTTP-запросов к сервису проверки грамматики
...
# Состояние по словам и проверке грамматики
user_state: Dict[int, Dict[str, Any]] = defaultdict(
    lambda: {
        ...
        "check_mode": False,    # режим проверки грамматики
    }
)
...
def load_words(path: str = "words.json") -> None:
    ...
    Ожидаемые форматы:
    1) [ { "de": "...", "tr": "...", "ru": "...", "topic": "..." }, ... ]
    2) [ { "topic": "...", "words": [ {de,tr,ru}, ... ] }, ... ]
    3) { "topics": [ { "topic": "...", "words": [...] }, ... ] }
    """
    ...
    with file_path.open("r", encoding="utf-8") as f:
        ...
        def add_word(raw: Dict[str, Any], topic_raw: str) -> None:
            """
            Добавляет одно слово. Тему берем из файла words.json.
            """
            de = raw.get("de")
            tr = raw.get("tr")
            ru = raw.get("ru")
            ...
            topic_name = (topic_raw or "").strip()

            # Пробуем сопоставить с известными темами
            topic_mapped = TOPIC_NAME_MAP.get(topic_name, topic_name or TOPIC_DICT)
            topic = topic_mapped
...
# ==========================
# ФУНКЦИИ ДЛЯ ПРОВЕРКИ ГРАММАТИКИ
# ==========================

async def check_grammar_de(text: str) -> tuple[str, List[Dict[str, Any]]]:
    """
    Отправляет немецкий текст в LanguageTool и возвращает:
    - исправленный вариант предложения
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data) as resp:
                if resp.status != 200:
                    print("Ошибка LanguageTool:", resp.status)
                    return text, []
                result = await resp.json()
    except Exception as e:
        print("Ошибка при обращении к LanguageTool:", e)
        return text, []

    matches = result.get("matches", [])

    # Строим исправленный текст
    ...
    for m in matches:
        ...
        context = text[offset:offset + length]
...
def apply_corrections(original: str, matches: List[Dict[str, Any]]) -> str:
    """
    Строим исправленный текст, применяя предложения LanguageTool.
    Берем первый вариант замены для каждого совпадения.
    """
    if not matches:
        return original

    # Сортируем по позиции в тексте
    matches_sorted = sorted(matches, key=lambda m: m.get("offset", 0))
    ...
    for m in matches_sorted:
        ...
        if offset < last_index:
            # Пересекающееся совпадение пропускаем
            continue

        result_parts.append(original[last_index:offset])

        repl_list = m.get("replacements", [])
        if repl_list:
            repl_value = repl_list[0].get("value", original[offset:offset + length])
            result_parts.append(repl_value)
        else:
            result_parts.append(original[offset:offset + length])

        last_index = offset + length

    result_parts.append(original[last_index:])
...
async def cmd_start(message: Message) -> None:
    ...
    if uid != ADMIN_ID and uid not in allowed_users:
        text = (
            ...
            "• Проверять грамматику твоих немецких предложений\n\n"
    ...
    text = (
        ...
        "Этот бот помогает учить немецкий язык через слова, грамматику и проверку предложений.\n\n"
        "📚 Возможности бота:\n"
        "• Учить слова по темам\n"
        ...
        "• /grammar - грамматика\n"
        "• /check - режим проверки грамматики немецких предложений\n"
        "• /checkoff - выйти из режима проверки грамматики\n\n"
        "🧠 Правило тренировки слов:\n"
        ...
        "📝 Проверка грамматики:\n"
        "1) Набери /check\n"
        "2) Отправляй свои предложения на немецком одним сообщением\n"
        "3) Бот покажет исправленный вариант и список ошибок\n"
        "4) Чтобы выйти, используй /checkoff\n\n"
...
async def cmd_check(message: Message) -> None:
    """
    Включает режим проверки грамматики.
    После этой команды все обычные текстовые сообщения
    будут проверяться как немецкие предложения.
    """
    uid = message.from_user.id
    ...
    await message.answer(
        "✍ Режим проверки грамматики включен.\n\n"
        "Отправь мне предложение или несколько предложений на немецком одним сообщением.\n"
        "Я проверю текст, покажу исправленный вариант и укажу ошибки.\n\n"
        "Чтобы выйти из режима проверки, используй команду /checkoff."
    )


@dp.message(Command("checkoff"))
async def cmd_checkoff(message: Message) -> None:
    """
    Выключает режим проверки грамматики.
    """
    uid = message.from_user.id
    ...
    await message.answer("Режим проверки грамматики отключен. Теперь бот снова работает как обычно.")
...
async def cb_allow_user(callback: CallbackQuery) -> None:
    ...
    try:
        text = (
            ...
            "• Проверяет грамматику твоих немецких предложений\n\n"
            "Режимы тренировки слов:\n"
            ...
            "• /next - следующее слово в текущей теме\n"
            "• /grammar - грамматика\n"
            "• /check - включить режим проверки грамматики предложений\n"
            "• /checkoff - выключить режим проверки грамматики\n\n"
            "Важно:\n"
            "Если ответ в викторине по словам неправильный, новое слово не дается.\n"
            "Нужно ответить правильно на текущее слово.\n"
            "После правильного ответа бот покажет полный перевод с транскрипцией.\n\n"
            "В режиме /check бот ожидает от тебя предложения на немецком и проверяет их грамматику."
        )
...
# ==========================
# ОБЩИЙ ХЕНДЛЕР ДЛЯ ТЕКСТА (ПРОВЕРКА ГРАММАТИКИ)
# ==========================
...
async def handle_text(message: Message) -> None:
    ...
    Если включен режим check_mode, то это запрос на проверку грамматики.
    """
    uid = message.from_user.id
    ...
    # Команды не трогаем, их обрабатывают другие хендлеры
    if text.startswith("/"):
        return

    state = user_state[uid]
    if not state.get("check_mode", False):
        # В обычном режиме текст игнорируем
        return

    # Режим проверки грамматики
    await message.answer("Проверяю грамматику, подожди секунду...")

    corrected, issues = await check_grammar_de(text)

    if not issues:
        await message.answer(
            "✅ Я не нашел ошибок в этом предложении.\n\n"
            f"Твой текст:\n{text}"
        )
        return
    ...
    lines: List[str] = []
    lines.append("⚠ Найдены возможные ошибки.")
    ...
    for i, issue in enumerate(issues, start=1):
        ...
        suggestion = repls[0] if repls else ""
        line = f"{i}) «{context}» → «{suggestion}»\n   {msg}"
        lines.append(line)
