...
def load_grammar_rules() -> None:
    """Загружаем все правила грамматики из grammar.json.

    Форматы, которые поддерживаются:
    1) Простой список:
       [
         { "id": "...", "level": "A1", "topic": "A1.1 - ...", "title": "...", "explanation": "..." },
         ...
       ]

    2) Словарь с ключом rules:
       { "rules": [ {...}, {...} ] }

    3) Общий словарь, где значения - списки правил:
       { "A1": [ {...}, {...} ], "A2": [ {...}, {...} ] }
    """
...
def get_sublevel_from_topic(topic: str) -> str:
    """Например: 'A1.1 - Базовые структуры' -> 'A1.1'."""
    if "—" in topic:
...
def load_words(path: str = "words.json") -> None:
    """Загружаем все слова из words.json.
