# bot.py
import asyncio
import json
import os
import random
from dataclasses import dataclass, field
from pathlib import Path
from typing import Dict, List, Optional

from aiogram import Bot, Dispatcher, F
from aiogram.filters import CommandStart, Command
from aiogram.types import Message, CallbackQuery
from aiogram.utils.keyboard import InlineKeyboardBuilder

# =============== НАСТРОЙКИ ===============

TOKEN = os.getenv("BOT_TOKEN", "PASTE_YOUR_TOKEN_HERE")
WORDS_FILE = Path("words.json")

ALL_TOPIC_KEY = "ALL"  # спец ключ для режима "все темы"


# =============== МОДЕЛИ И ЗАГРУЗКА ===============

@dataclass
class Word:
    id: int
    de: str
    tr: str
    ru: str
    topic: str


WORDS: List[Word] = []
WORDS_BY_ID: Dict[int, Word] = {}
TOPIC_IDS: Dict[str, List[int]] = {}
TOPIC_COUNTS: Dict[str, int] = {}
TOTAL_WORDS: int = 0


def detect_topic(item: Dict[str, str]) -> str:
    """Определяем тему по русскому переводу."""
    ru = item.get("ru", "").lower()
    de = item.get("de", "").lower()

    # приветствия и базовые фразы
    if any(k in ru for k in [
        "привет", "добрый день", "добрый вечер", "доброе утро",
        "доброй ночи", "как дела", "до свидания", "спасибо",
        "пожалуйста", "извини", "мне жаль", "да", "нет"
    ]):
        return "Приветствия и базовые фразы"

    # личные данные
    if any(k in ru for k in [
        "имя", "фамилия", "улица", "адрес", "почтовый индекс",
        "место проживания", "номер телефона", "подпись",
        "возраст", "семейное положение", "почта", "электронный адрес"
    ]):
        return "Личные данные"

    # семья
    if any(k in ru for k in [
        "семья", "мать", "отец", "сын", "дочь", "братья и сестры",
        "бабушка", "дед", "внук", "внучка", "мужчина / муж",
        "тетя", "дядя", "двоюрод", "подруга", "друг"
    ]):
        return "Семья"

    # профессии и работа
    if any(k in ru for k in [
        "врач", "учитель", "инженер", "повар", "медбрат", "медсестра",
        "таксист", "продавец", "парикмахер", "официант", "актер",
        "фотограф", "домохозя", "полицейск", "студент", "сотрудник",
        "профессия", "работа", "офис", "фирма", "зарплата",
        "собеседование", "заявление на работу"
    ]):
        return "Профессии и работа"

    # базовые глаголы
    if any(k in ru for k in [
        "быть", "иметь", "жить", "говорить", "учиться", "работать",
        "есть", "готовить", "искать", "идти", "звонить", "думать",
        "делать", "считать", "произносить", "любить", "хотел бы",
        "нуждаться", "давать", "покупать", "путешествовать",
        "садиться", "сидеть", "стоять", "лежать", "бежать",
        "прыгать", "забывать", "вспоминать", "показывать", "ждать",
        "мыть", "убирать", "чинить", "резать", "открывать", "закрывать",
        "носить", "бросать", "светить", "считать", "тренироваться"
    ]):
        return "Базовые глаголы"

    # школа и учеба
    if any(k in ru for k in [
        "книга", "тетрадь", "домашнее задание", "урок", "класс",
        "экзамен", "тест", "повторять", "объяснять", "понимать",
        "курс", "школа", "университет", "школьник", "студент"
    ]):
        return "Школа и учеба"

    # предметы и вещи
    if any(k in ru for k in [
        "бумага", "карандаш", "ручка", "линейка", "камера",
        "принтер", "ноутбук", "телефон", "сумка", "рюкзак",
        "кошелек", "ключ", "ножницы", "зонт", "очки", "зажигалка",
        "газета", "чашка", "чемодан"
    ]):
        return "Предметы и вещи"

    # время и календарь
    if any(k in ru for k in [
        "понедельник", "вторник", "среда", "четверг", "пятница",
        "суббота", "воскресенье", "утро", "полдень",
        "первая половина дня", "вторая половина дня",
        "вечер", "ночь", "который час", "время", "день рождения",
        "год", "месяц", "градус", "температура", "неделя"
    ]):
        return "Время и календарь"

    # еда и магазин
    if any(k in ru for k in [
        "кофе", "чай", "молоко", "вода", "сок", "пиво", "хлеб",
        "булочка", "круассан", "яйцо", "яблоко", "груша", "фрукты",
        "мюсли", "йогурт", "торт", "колбаса", "сыр", "покупка",
        "евро", "цент", "клиент", "пакет", "супермаркет",
        "продукт питания", "овощи", "мясо", "напиток", "десерт",
        "еда", "любимая еда", "рис", "шоколад", "мороженое",
        "ужин", "обед", "завтрак", "масло", "рыба", "счет в ресторане",
        "веганский", "вегетарианский"
    ]):
        return "Еда и магазин"

    # погода и природа
    if any(k in ru for k in [
        "погода", "пасмурно", "идет дождь", "идет снег", "светит солнце",
        "тепло", "холодно", "весна", "лето", "осень", "зима", "градус",
        "температура", "лес", "река", "ручей", "холм", "луг",
        "поле", "скала", "пляж", "радуга", "шторм", "небо", "облако",
        "природа", "земля", "климат", "снег", "туман", "ветер"
    ]):
        return "Погода и природа"

    # дом и жилье
    if any(k in ru for k in [
        "дом", "квартира", "комната", "гостиная", "спальня",
        "кухня", "ванная", "туалет", "коридор", "балкон", "сад",
        "окно", "дверь", "стол", "стул", "кровать", "шкаф",
        "лампа", "ковер", "телевизор", "стиральная машина",
        "холодильник", "плита", "духовка", "душ", "ванна (как предмет)",
        "пылесос", "ведро", "мусор", "мусорный пакет", "мусорный бак"
    ]):
        return "Дом и жилье"

    # город и транспорт
    if any(k in ru for k in [
        "город", "деревня", "улица", "площадь", "парк", "мост",
        "вокзал", "аэропорт", "остановка", "автобусная остановка",
        "метро", "городская электричка", "поезд", "автобус", "такси",
        "аптека", "пекарня", "банк", "почта", "полиция", "школа",
        "университет", "детская площадка", "кинотеатр", "театр",
        "музей", "больница", "порт", "рынок", "парковка", "район"
    ]):
        return "Город и транспорт"

    # тело и здоровье
    if any(k in ru for k in [
        "голова", "лицо", "лоб", "глаз", "бровь", "нос", "рот",
        "зуб", "ухо", "волос", "шея", "плечо", "рука", "кисть",
        "палец", "грудь", "спина", "живот", "нога", "стопа",
        "колено", "кость", "кровь", "легкое", "печень", "мышца",
        "здоровый", "больной", "уставший", "здоровье", "боль",
        "головная боль", "боль в горле", "простуда", "грипп",
        "температура", "таблетка", "лекарство", "мазь", "повязка",
        "бинт", "приём у врача", "пациент"
    ]):
        return "Тело и здоровье"

    # эмоции и характер
    if any(k in ru for k in [
        "счастливый", "грустный", "злой", "спокойный", "нервный",
        "расслабленный", "гордый", "застенчивый", "дружелюбный",
        "готовый помочь", "вежливый", "невежливый", "странный",
        "смешной", "серьезный", "скучный", "захватывающий",
        "интересный", "важный", "легкий", "сложный", "честный",
        "ленивый", "трудолюбивый", "смелый", "трусливый", "умный",
        "глупый", "спокойный", "громкий", "наглый", "терпеливый",
        "нетерпеливый", "симпатичный", "неприятный", "с чувством юмора",
        "успешный", "любопытный"
    ]):
        return "Эмоции и характер"

    # хобби и спорт
    if any(k in ru for k in [
        "спорт", "тренировка", "команда", "музыка", "слушать",
        "танцевать", "фотографировать", "пианино", "рисовать",
        "шить", "плавать", "петь", "гитара", "видео", "хобби",
        "фильм", "сериал", "велосипед", "игра", "выигрывать"
    ]):
        return "Хобби и спорт"

    # животные
    if any(k in ru for k in [
        "животное", "собака", "кошка", "птица", "лошадь",
        "корова", "свинья", "овца", "мышь", "медведь", "лев",
        "змея", "тигр", "заяц", "обезьяна", "верблюд", "волк",
        "лиса", "петух", "утка"
    ]):
        return "Животные"

    # одежда
    if any(k in ru for k in [
        "рубашка", "штаны", "джинсы", "футболка", "свитер",
        "куртка", "пальто", "блузка", "юбка", "платье", "обувь",
        "ботинок", "туфля", "носки", "сапоги", "ремень", "шляпа",
        "шапка", "шарф", "перчатки", "трусы", "лифчик", "одежда",
        "мода", "ткань", "пуговица", "молния", "размер"
    ]):
        return "Одежда"

    # цвета и числа
    if any(k in ru for k in [
        "цвет", "красный", "синий", "зеленый", "желтый", "черный",
        "белый", "серый", "коричневый", "оранжевый", "фиолетовый",
        "розовый", "ноль", "один", "два", "три", "четыре", "пять",
        "шесть", "семь", "восемь", "девять", "десять", "одиннадцать",
        "двенадцать", "двадцать", "тридцать", "сорок", "пятьдесят",
        "шестдесят", "семьдесят", "восемьдесят", "девяносто", "сто"
    ]):
        return "Цвета и числа"

    # компьютер и интернет
    if any(k in ru for k in [
        "компьютер", "ноутбук", "мышь", "клавиатура", "экран",
        "файл", "папка", "сохранять", "удалять", "пароль", "входить",
        "выходить", "программа", "приложение", "скачивать",
        "загружать", "интернет", "вай-фай", "печатать"
    ]):
        return "Компьютер и интернет"

    # инструменты и быт
    if any(k in ru for k in [
        "пылесос", "метла", "ведро", "губка", "тряпка",
        "розетка", "вилка", "лампочка", "микроволновка",
        "чайник электрический", "кастрюля", "сковорода",
        "мусорный пакет", "мусорный бак", "балкон", "вход",
        "молоток", "гвоздь", "винт", "отвертка", "дрель",
        "пила", "плоскогубцы", "шланг", "клей", "инструмент"
    ]):
        return "Инструменты и быт"

    # абстрактные понятия
    if any(k in ru for k in [
        "идея", "мечта", "сон", "желание", "возможность", "проблема",
        "решение", "вопрос", "ответ", "будущее", "прошлое",
        "настоящее", "страница", "сторона", "начало", "конец",
        "середина", "причина", "ошибка", "опыт", "помощь", "цель",
        "информация", "надежда", "уважение", "сомнение", "терпение"
    ]):
        return "Абстрактные понятия"

    # по умолчанию
    return "Словарь A1-B1"


def load_words() -> None:
    """Загружаем слова из words.json и назначаем темы."""
    global WORDS, WORDS_BY_ID, TOPIC_IDS, TOPIC_COUNTS, TOTAL_WORDS

    with WORDS_FILE.open("r", encoding="utf-8") as f:
        data = json.load(f)

    WORDS = []
    WORDS_BY_ID = {}
    TOPIC_IDS = {}
    TOPIC_COUNTS = {}

    for idx, item in enumerate(data):
        topic = item.get("topic")
        if not topic:
            topic = detect_topic(item)

        w = Word(
            id=idx,
            de=item["de"],
            tr=item.get("tr", ""),
            ru=item["ru"],
            topic=topic,
        )

        WORDS.append(w)
        WORDS_BY_ID[w.id] = w
        TOPIC_IDS.setdefault(topic, []).append(w.id)
        TOPIC_COUNTS[topic] = TOPIC_COUNTS.get(topic, 0) + 1

    TOTAL_WORDS = len(WORDS)
    print(f"Loaded {TOTAL_WORDS} words, topics: {len(TOPIC_IDS)}")


# =============== СОСТОЯНИЕ ПОЛЬЗОВАТЕЛЯ ===============

@dataclass
class UserState:
    mode: str = "de_ru"        # de_ru, ru_de, mixed
    topic: str = ALL_TOPIC_KEY # ALL или конкретная тема
    remaining_ids: List[int] = field(default_factory=list)
    current_word_id: Optional[int] = None
    correct: int = 0
    wrong: int = 0

    def reset_stats(self) -> None:
        self.correct = 0
        self.wrong = 0

    def reset_pool(self) -> None:
        if self.topic == ALL_TOPIC_KEY:
            ids = [w.id for w in WORDS]
        else:
            ids = TOPIC_IDS.get(self.topic, [])
        random.shuffle(ids)
        self.remaining_ids = ids
        self.current_word_id = None


USER_STATES: Dict[int, UserState] = {}


def get_user_state(user_id: int) -> UserState:
    state = USER_STATES.get(user_id)
    if state is None:
        state = UserState()
        state.reset_pool()
        USER_STATES[user_id] = state
    return state


# =============== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ===============

def get_topics_ordered() -> List[str]:
    return sorted(TOPIC_IDS.keys())


def format_word_question_de_ru(w: Word) -> str:
    return f"{w.de} [{w.tr}]" if w.tr else w.de


def format_word_full_de(w: Word) -> str:
    if w.tr:
        return f"{w.de} [{w.tr}]"
    return w.de


def pick_wrong_answers(correct_word: Word, direction: str, count: int = 3) -> List[str]:
    """Подбираем неправильные варианты того же типа что и правильный."""
    if direction == "de_ru":
        correct = correct_word.ru
        pool = [w.ru for w in WORDS if w.id != correct_word.id]
    else:
        correct = format_word_full_de(correct_word)
        pool = [format_word_full_de(w) for w in WORDS if w.id != correct_word.id]

    pool = list(set(pool))
    if correct in pool:
        pool.remove(correct)

    if len(pool) <= count:
        return random.sample(pool, k=min(len(pool), count))

    return random.sample(pool, k=count)


async def send_stats(message: Message, state: UserState) -> None:
    total = state.correct + state.wrong
    if total == 0:
        await message.answer("Статистика пока пустая. Сначала ответь хотя бы на один вопрос.")
        return

    topic_name = "Все темы" if state.topic == ALL_TOPIC_KEY else state.topic

    text = (
        f"Тема: <b>{topic_name}</b>\n"
        f"Всего вопросов: <b>{total}</b>\n"
        f"Правильных ответов: <b>{state.correct}</b>\n"
        f"Ошибок: <b>{state.wrong}</b>"
    )
    await message.answer(text)


async def send_next_question(target, user_id: int) -> None:
    """target - это Message или CallbackQuery.message."""
    state = get_user_state(user_id)

    # если слова в теме закончились, показываем статистику и перезапускаем
    if not state.remaining_ids:
        await send_stats(target, state)
        state.reset_stats()
        state.reset_pool()
        await target.answer(
            "Ты ответил на все слова в этой теме. Статистика выше.\n"
            "Я заново перемешал слова. Можешь продолжать с той же темой или выбрать другую через /themes."
        )
        return

    # берем следующее слово
    word_id = state.remaining_ids.pop()
    state.current_word_id = word_id
    word = WORDS_BY_ID[word_id]

    # выбираем направление
    if state.mode == "mixed":
        direction = random.choice(["de_ru", "ru_de"])
    else:
        direction = state.mode

    if direction == "de_ru":
        question_text = format_word_question_de_ru(word)
        correct_option = word.ru
    else:
        question_text = word.ru
        correct_option = format_word_full_de(word)

    wrong_options = pick_wrong_answers(word, direction, count=3)
    options = [correct_option] + wrong_options
    random.shuffle(options)

    kb = InlineKeyboardBuilder()
    for option in options:
        is_correct = 1 if option == correct_option else 0
        cb_data = f"ans:{word_id}:{is_correct}"
        kb.button(text=option, callback_data=cb_data)
    kb.adjust(2)

    text = f"Выбери правильный перевод:\n\n<b>{question_text}</b>"
    await target.answer(text, reply_markup=kb.as_markup())


# =============== ИНИЦИАЛИЗАЦИЯ БОТА ===============

bot = Bot(TOKEN, parse_mode="HTML")
dp = Dispatcher()


# =============== КОМАНДЫ ===============

@dp.message(CommandStart())
async def cmd_start(message: Message) -> None:
    state = get_user_state(message.from_user.id)
    state.reset_stats()
    state.reset_pool()

    topics_count = len(TOPIC_IDS)

    text = (
        "🇩🇪 Привет! Это бот для изучения немецких слов.\n\n"
        "Как пользоваться:\n"
        "• Я показываю слово и 4 варианта перевода.\n"
        "• Нажми на кнопку с вариантом.\n"
        "• Если ответ неверный, я покажу правильный ответ и сразу дам новое слово.\n"
        "• Если ответ верный, я просто покажу следующее слово.\n\n"
        f"Сейчас в базе <b>{TOTAL_WORDS}</b> слов.\n"
        f"Тем: <b>{topics_count}</b>.\n\n"
        "Режимы:\n"
        "• 🇩🇪 → 🇷🇺 немецкое слово и варианты на русском.\n"
        "• 🇷🇺 → 🇩🇪 русское слово и варианты на немецком.\n"
        "• 🎲 смешанный режим.\n\n"
        "Команды:\n"
        "/next - следующее слово\n"
        "/themes - выбрать тему слов\n"
        "/mode - выбрать направление перевода\n"
        "/stats - статистика по текущей теме\n\n"
        "По умолчанию включен режим 🇩🇪 → 🇷🇺 и все темы вперемешку.\n"
        "Напиши /next, чтобы начать квиз."
    )
    await message.answer(text)


@dp.message(Command("next"))
async def cmd_next(message: Message) -> None:
    await send_next_question(message, message.from_user.id)


@dp.message(Command("stats"))
async def cmd_stats(message: Message) -> None:
    state = get_user_state(message.from_user.id)
    await send_stats(message, state)


@dp.message(Command("mode"))
async def cmd_mode(message: Message) -> None:
    kb = InlineKeyboardBuilder()
    kb.button(text="🇩🇪 → 🇷🇺", callback_data="mode:de_ru")
    kb.button(text="🇷🇺 → 🇩🇪", callback_data="mode:ru_de")
    kb.button(text="🎲 Смешанный", callback_data="mode:mixed")
    kb.adjust(1)

    await message.answer("Выбери направление перевода:", reply_markup=kb.as_markup())


@dp.message(Command("themes"))
async def cmd_themes(message: Message) -> None:
    kb = InlineKeyboardBuilder()

    kb.button(text="🎲 Все темы (перемешку)", callback_data=f"topic:{ALL_TOPIC_KEY}")

    for topic in get_topics_ordered():
        count = TOPIC_COUNTS.get(topic, 0)
        kb.button(text=f"{topic} ({count})", callback_data=f"topic:{topic}")

    kb.adjust(1)
    await message.answer("Выбери тему слов:", reply_markup=kb.as_markup())


# =============== CALLBACK КНОПКИ ===============

@dp.callback_query(F.data.startswith("mode:"))
async def callback_mode(call: CallbackQuery) -> None:
    await call.answer()
    mode = call.data.split(":", 1)[1]
    state = get_user_state(call.from_user.id)

    if mode not in {"de_ru", "ru_de", "mixed"}:
        await call.message.answer("Неизвестный режим.")
        return

    state.mode = mode
    names = {
        "de_ru": "🇩🇪 → 🇷🇺 Немецкий на русский",
        "ru_de": "🇷🇺 → 🇩🇪 Русский на немецкий",
        "mixed": "🎲 Смешанный режим",
    }
    await call.message.answer(
        f"Режим изменен на: <b>{names[mode]}</b>.\nНапиши /next, чтобы продолжить."
    )


@dp.callback_query(F.data.startswith("topic:"))
async def callback_topic(call: CallbackQuery) -> None:
    await call.answer()
    topic_key = call.data.split(":", 1)[1]
    state = get_user_state(call.from_user.id)

    if topic_key != ALL_TOPIC_KEY and topic_key not in TOPIC_IDS:
        await call.message.answer("Эта тема не найдена.")
        return

    state.topic = topic_key
    state.reset_stats()
    state.reset_pool()

    if topic_key == ALL_TOPIC_KEY:
        title = "Все темы (перемешку)"
        count = TOTAL_WORDS
    else:
        title = topic_key
        count = TOPIC_COUNTS.get(topic_key, 0)

    await call.message.answer(
        f"Тема изменена на <b>{title}</b>.\n"
        f"Слов в этой теме: <b>{count}</b>.\n"
        "Напиши /next, чтобы начать."
    )


@dp.callback_query(F.data.startswith("ans:"))
async def callback_answer(call: CallbackQuery) -> None:
    try:
        _, word_id_str, is_correct_str = call.data.split(":")
        word_id = int(word_id_str)
        is_correct = is_correct_str == "1"
    except Exception:
        await call.answer("Ошибка данных.", show_alert=True)
        return

    state = get_user_state(call.from_user.id)

    if state.current_word_id != word_id:
        await call.answer("Этот вопрос уже неактуален. Нажми /next.", show_alert=True)
        return

    word = WORDS_BY_ID.get(word_id)
    if not word:
        await call.answer("Слово не найдено.", show_alert=True)
        return

    if is_correct:
        state.correct += 1
        await call.answer("Верно!")
        await send_next_question(call.message, call.from_user.id)
    else:
        state.wrong += 1
        await call.answer("Неверно.")

        text = (
            "❌ Неверно.\n"
            "Правильный ответ:\n"
            f"🇩🇪 <b>{format_word_full_de(word)}</b>\n"
            f"🇷🇺 <b>{word.ru}</b>"
        )
        await call.message.answer(text)
        await send_next_question(call.message, call.from_user.id)


# =============== ТОЧКА ВХОДА ===============

async def main() -> None:
    load_words()
    await dp.start_polling(bot)


if __name__ == "__main__":
    asyncio.run(main())
